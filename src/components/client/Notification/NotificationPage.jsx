...
import useNotification from "@/hooks/leader/notification/useNotification";
...
function Notification_Page({ onUnreadChange }) {
  ...
  useEffect(() => {
    if (!loadedRef.current) {
      fetchNotifications(1, onUnreadChange);
  ...
  const loadMore = () => {
    ...
    fetchNotifications(page + 1, onUnreadChange);
    setPage((p) => p + 1);
  };
    ...
    for (let i = 1; i <= totalPages; i++) {
      pages.push(
        <button
          ...
          onClick={() => fetchNotifications(i, onUnreadChange)}
          disabled={i === page}
        >
          {i}
        </button>
