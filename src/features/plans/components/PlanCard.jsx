...
export default function PlanCard({
  ...
  const pricePerMonthRaw = isFreePlan ? 0 : (isAnnual ? plan.priceYear / 12 : plan.priceMonth);
  const hasSale = !isFreePlan && isAnnual && plan.sale > 0;
  const pricePerMonth = hasSale
    ? pricePerMonthRaw * (1 - plan.sale / 100)
    : pricePerMonthRaw;
  const billedTotal = isFreePlan ? 0 : (isAnnual ? pricePerMonth * 12 : pricePerMonth);
  ...
  const hasOtherPlan = normalizedCurrentSlug && 
  ...
  const isDisabled = lockedByRole || (baseDisabled && !allowRenew);
    ...
    >
      ...
      <div className="mb-3">
        <div className="flex items-baseline gap-1.5">
          <span className="text-2xl font-extrabold text-text-strong">
            {formatMoney(pricePerMonth, currency, locale)}
          </span>
          <span className="text-text-muted text-xs">/tháng</span>
        </div>
      ...
      <ul className="mt-2 space-y-1.5 text-[13px] flex-1 overflow-y-auto min-h-0">
        {plan.description.map((line, idx) => (
          <li key={idx} className="flex items-start gap-1.5 text-text-strong">
      ...
      <button
        className={`w-full mt-4 py-2 rounded-lg text-sm font-semibold shadow transition ${
          isDisabled
            ? "bg-surface-soft text-text-strong cursor-not-allowed opacity-60"
            : "bg-brand text-[var(--color-surface-50)] hover:opacity-90"
        }`}
        onClick={() => {
          if (lockedByRole) return;
          if (allowRenew) {
