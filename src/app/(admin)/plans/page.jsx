        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 md:gap-6 mb-6">
          {pagedPlans.map((plan) => {
            ...
            return (
              <div
                key={plan._id}
                className="bg-white border-2 border-[#1cadd9] rounded-xl shadow-lg p-4 md:p-6 flex flex-col h-full relative group hover:shadow-xl transition-all duration-300"
              >
                ...
                <div className="flex items-center justify-center mb-3 pt-2">
                  <span className="text-2xl md:text-3xl mr-2">{plan.icon}</span>
                  <span className="font-bold text-lg md:text-xl text-center">
                    {plan.name}
                  </span>
                </div>

                {/* Thông tin dung lượng và người dùng */}
                <div className="grid grid-cols-2 gap-3 mb-4 text-sm">
                  <div className="text-center p-2 bg-gray-50 rounded-lg">
                    <div className="text-gray-600 text-xs mb-1">Dung lượng</div>
                    <div className="font-semibold text-gray-900">
                      {(plan.storage / (1024 * 1024 * 1024)).toFixed(1)} GB
                    </div>
                  </div>
                  <div className="text-center p-2 bg-gray-50 rounded-lg">
                    <div className="text-gray-600 text-xs mb-1">Người dùng</div>
                    <div className="font-semibold text-gray-900">
                      {plan.users}
                    </div>
                  </div>
                ...
                <div className="mb-2 text-sm text-gray-700">
                  {plan.sale > 0 && plan.priceMonth > 0 ? (
                    <div className="flex items-center gap-2">
                      <span>Thanh toán tháng:</span>
                      <span className="line-through text-gray-400">
                        {Math.round(plan.priceMonth / 1000)}K
                      </span>
                      <span className="text-gray-400">→</span>
                      <span className="font-bold text-primary text-base">
                        {Math.round(priceMonthDiscounted / 1000)}K
                      </span>
                    </div>
                  ) : (
                    <div className="flex items-center gap-2">
                ...
                <div className="mb-4 text-sm text-gray-700">
                  {plan.sale > 0 && plan.priceYear > 0 ? (
                    <div className="flex items-center gap-2">
                      <span>Thanh toán năm:</span>
                      <span className="line-through text-gray-400">
                        {Math.round(plan.priceYear / 1000)}K
                      </span>
                      <span className="text-gray-400">→</span>
                      <span className="font-bold text-primary text-base">
                        {Math.round(priceYearDiscounted / 1000)}K
                      </span>
                    </div>
                  ) : (
                    <div className="flex items-center gap-2">
                ...
                {/* Features */}
                <ul className="flex-1 space-y-1.5 mb-4 text-gray-700 text-xs md:text-sm">
