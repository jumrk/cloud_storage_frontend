...
export default function MemberFileManager() {
  ...
  const fetchFolders = async () => {
    ...
    setError("");
    try {
      const res = await fetch("/api/member/folders");
  ...
  // Helper: tìm folder theo id trong cây
  ...
  // Xử lý rename, delete, move (tùy quyền, sẽ bổ sung sau)
  const handleRename = (id, type, newName) => {
    toast("Chức năng đổi tên sẽ tích hợp sau!");
  };
  const handleMoveItem = (items, targetFolderId) => {
    toast("Chức năng di chuyển sẽ tích hợp sau!");
  };
  ...
  const confirmMove = async () => {
    if (!pendingMoveItems || pendingMoveItems.length === 0 || !moveTargetFolder)
      return;
    ...
    try {
      ...
      if (data.success) {
        ...
        toast.error(data.error || "Di chuyển thất bại!");
      }
    } catch (err) {
  ...
  const normalizedData = folderChildren.map((item) => ({
    id: item._id,
            ...
            <div className="w-full grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
              {normalizedData.length === 0 && (
                <div className="col-span-full text-center text-gray-400 py-8">
                  Không có file hoặc thư mục nào
                </div>
              )}
