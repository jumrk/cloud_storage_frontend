...
export default function HeaderDetailBoard() {
  ...
  return (
    <div className="flex justify-between w-full h-[60px] items-center">
      <button
        onClick={() => router.back()}
        className="flex gap-1 p-1 rounded-xl border border-border text-text-muted hover:text-text-strong hover:bg-surface-50 transition"
      ...
      <div className="flex gap-2 justify-end items-center">
        <p className="box-content px-3 py-2 rounded-2xl border border-border text-text-strong">
          {title}
        </p>
        {isOwner && (
          <button
            onClick={() => router.push(`/job-management/workspace/board/${boardId}/report`)}
            className="flex items-center gap-2 rounded-2xl border border-border px-4 py-2 text-sm font-medium text-text-strong hover:bg-surface-50 transition"
          >
