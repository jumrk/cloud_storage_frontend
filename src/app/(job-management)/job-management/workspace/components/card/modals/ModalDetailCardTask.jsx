...
function ModalDetailCardTask({ open, card, onClose, onSave, boardId }) {
  ...
  return (
    ...
    >
      ...
      <div
        ref={panelRef}
        className="relative z-10 w-full max-w-6xl max-h-[calc(100vh-80px)] overflow-hidden flex flex-col rounded-2xl border border-border bg-white shadow-2xl"
      ...
      >
        ...
        <div className="px-4 flex-1 overflow-hidden py-4">
          <div className="grid grid-cols-1 md:grid-cols-2 h-full gap-4">
            <div className="flex flex-col rounded-xl border border-dashed border-border scrollbar-hide overflow-y-auto">
              <div className="sticky flex items-center gap-2 top-0 z-10 bg-white/95 backdrop-blur supports-[backdrop-filter]:bg-white/70 border-b border-border px-3 py-3">
                <span className="inline-block h-3.5 w-3.5 rounded-full border-2 border-border" />
                {!editingTitle ? (
                  <h2
                    className="text-2xl font-semibold text-text-strong cursor-text hover:opacity-90"
                    title={t("job_management.board.click_to_edit_title")}
                    onClick={() => setEditingTitle(true)}
                  >
                    {titleDraft || card?.title || t("job_management.board.no_title")}
                  </h2>
                ) : (
                  <input
                    ref={titleInputRef}
                    type="text"
                    className="text-2xl font-semibold text-text-strong bg-white border-b border-border focus:border-brand-400 outline-none rounded-none px-1 -mx-1"
                    value={titleDraft}
                ...
                <div className="space-y-5 text-sm">
                      ...
                      {!hasLabels && (
                        <div className="relative">
                          <button
                            ...
                            className="inline-flex items-center gap-2 rounded-lg border border-border bg-white hover:bg-surface-50 px-3 py-1.5 text-text-strong"
                          >
                            <IoPricetagOutline /> {t("job_management.board.color")}
                          </button>
                          <LabelPopover
                            open={labelsOpen}
                  ...
                  <div className="relative">
                    <button
                      ...
                      className="inline-flex items-center gap-2 rounded-lg border border-border bg-white hover:bg-surface-50 px-3 py-1.5 text-text-strong"
                    >
                      <IoCheckboxOutline /> {t("job_management.checklist.tasks_to_do")}
                    </button>
                    {addingChecklist && (
                      <div className="absolute mt-2 z-10 w-80 rounded-xl border border-border bg-white p-3 shadow-xl">
                        <div className="text-sm mb-2 text-text-strong">
                        ...
                        <input
                          ...
                          className="w-full h-10 rounded-lg border border-border bg-white px-3 outline-none focus:border-brand-400"
                        />
                        ...
                        <div className="mt-3 flex items-center gap-2">
                          ...
                          <button
                            className="h-9 px-4 rounded-lg border border-border bg-white text-text-strong text-sm hover:bg-surface-50"
                            onClick={() => {
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <div className="relative">
                            <button
                              type="button"
                              onClick={() => setMembersOpen(true)}
                              className="h-7 w-7 rounded-md bg-surface-50 hover:bg-white border border-border flex items-center justify-center"
                    ...
                    {hasLabels && (
                      <div className="space-y-2">
                        <div className="flex items-center gap-2 text-text-muted">
                          <IoPricetagOutline />
                          <span className="font-medium text-sm">{t("job_management.board.color")}</span>
                        </div>
                  ...
                  {hasStart && (
                    <div className="space-y-2">
                      ...
                      <div className="flex items-center gap-2">
                        <span className="inline-flex items-center h-8 px-3 rounded-md bg-surface-50 text-text-strong text-sm">
                          {formatStartBadge()}
                        </span>
                        <div className="relative">
                          <button
                            type="button"
                            onClick={() => setStartOpen(true)}
                            className="h-8 w-8 rounded-md bg-surface-50 hover:bg-white border border-border grid place-items-center"
                    ...
                    {descOpen ? (
                      ...
                      <div>
                        <div className="flex items-center justify-between">
                          <div className="flex items-center gap-2 text-text-muted">
                            <IoDocumentTextOutline />
                            <span className="font-medium text-sm">{t("job_management.board.description")}</span>
                          </div>
                          <button
                            type="button"
                        ...
                        {!descOpen && hasDescription && (
                          <div
                            className="px-1 text-text-strong prose prose-sm max-w-none [&_ul]:list-disc [&_ol]:list-decimal [&_ul,&_ol]:pl-5 [&_blockquote]:border-l-4 [&_blockquote]:pl-3"
                            dangerouslySetInnerHTML={{ __html: previewHtml }}
                          />
                        )}

                        {!descOpen && !hasDescription && (
                          <div className="w-full rounded-lg border border-border bg-white text-text-muted px-4 py-3">
            ...
            <div className="overflow-auto scrollbar-hide rounded-xl border border-dashed border-border p-3">
              <div className="space-y-4 text-sm">
                <div className="flex items-center justify-between">
                  <div className="inline-flex items-center gap-2">
                    <IoChatbubbleOutline className="text-text-muted" />
                ...
                <form onSubmit={handleAddComment}>
                  <input
                    ...
                    className="w-full h-10 rounded-xl border border-border bg-white text-text-strong placeholder-text-muted px-4 outline-none focus:border-brand-400"
                  />
                </form>
                        ...
                        <div className="flex items-start gap-3">
                          <div className="flex items-start gap-3 w-full">
                            <div className="flex-1">
                              ...
                              <div className="mt-1 w-full px-3 py-2 text-[15px] text-text-strong">
                                {e?.text}
                              </div>
                            </div>
                          </div>
                          ...
                          <button
                            className="p-1 rounded-md text-text-muted hover:text-danger-600 hover:bg-danger-50 transition-colors"
