...
export default function MembersPopover({
  ...
  return (
    <div
      ref={panelRef}
      className={`absolute z-50 top-full mt-2 w-80 rounded-xl border border-border bg-white shadow-xl ${className}`}
      onClick={(e) => e.stopPropagation()}
      role="dialog"
      aria-label={t("job_management.card.members")}
    >
      <div className="flex items-center justify-between px-3 py-2 border-b border-border">
        <span className="font-medium text-text-strong">{t("job_management.card.members")}</span>
        <button
      ...
      <div className="p-3 space-y-3">
        <input
          ...
          className="w-full h-9 rounded-lg border border-border bg-white px-3 text-sm outline-none focus:border-brand-400"
        />

        <div className="text-xs text-text-muted">{t("job_management.card.board_members")}</div>

        <div className="max-h-64 overflow-auto divide-y divide-border">
          {filtered.length === 0 && (
            <div className="text-sm text-text-muted py-2">{t("job_management.card.not_found")}</div>
          )}
          {filtered.map((m) => (
            <label
              key={m.id}
