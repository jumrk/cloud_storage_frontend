...
export default function DuePopover({
  ...
  return (
    <div
      ref={panelRef}
      className={`absolute z-50 top-full mt-2 w-[22rem] rounded-2xl border border-border bg-white shadow-2xl ${className}`}
    ...
    >
      ...
      <div className="p-3 space-y-3">
        <div className="flex items-center justify-between">
          <button
            className="h-8 px-2 rounded-lg border border-border hover:bg-surface-50 text-sm"
        ...
        <div className="grid grid-cols-7 gap-1 text-xs text-text-muted select-none">
          {dowShort.map((d) => (
        ...
        <div className="flex items-center gap-2">
          <div className="text-xs text-text-muted w-16">{t("job_management.board.time")}</div>
          <select
            disabled={!enabled}
            value={hour}
            onChange={(e) => setHour(e.target.value)}
            className="h-9 flex-1 rounded-lg border border-border bg-white px-3 text-sm outline-none focus:border-brand-400 disabled:opacity-50"
          >
            {hourOpts.map((h) => (
          ...
          <select
            ...
            className="h-9 w-28 rounded-lg border border-border bg-white px-3 text-sm outline-none focus:border-brand-400 disabled:opacity-50"
          >
            {minuteOpts.map((m) => (
        ...
        <div className="flex flex-wrap gap-2">
          <button
            ...
            className="h-8 px-3 rounded-lg border border-border text-sm hover:bg-surface-50 disabled:opacity-50"
          >
            Hôm nay
          </button>
          <button
        ...
        <div className="flex gap-2 pt-1">
          ...
          <button
            onClick={clearDue}
            className="h-9 px-3 rounded-lg border border-border text-sm hover:bg-surface-50"
          >
            Gỡ bỏ
          </button>
        </div>
