...
export default function ChecklistBlock({
  ...
  const onToggleItem = async (itemId, nextDone) => {
    ...
    try {
      ...
      if (!ok) {
        ...
        toast.error(res?.data?.messenger || t("job_management.errors.general_error"));
        return;
      }
      setLoading(false);
  ...
  const onUpdateItem = async (itemId, patch) => {
    ...
    try {
      ...
      if (!payload?.success) {
        setCheckListItems(prev);
        toast.error(payload?.messenger || t("job_management.errors.update_failed"));
        return;
      }
      if (payload.data) {
  ...
  const onDeleteItem = async (itemId) => {
    ...
    try {
      ...
      const ok = res?.data?.success;
      if (!ok) throw new Error(res?.data?.messenger || t("job_management.errors.general_error"));
      toast.success(t("job_management.success.delete_item_success"));
    } catch (e) {
      const msg = e?.response?.data?.messenger || t("job_management.errors.general_error");
      setCheckListItems(prev);
      toast.error(msg);
    }
  };
  ...
  return (
    <div className="p-3 border border-border border-dashed rounded-2xl w-full bg-white">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <input
            ...
            className="bg-transparent outline-none text-base font-medium text-text-strong"
          />
        </div>
        <div className="flex items-center gap-2">
          <button
      ...
      <div className="space-y-1">
        <div className="flex items-center gap-2 text-sm">
          <span className="w-10 tabular-nums text-text-muted">{percent}%</span>
          <div
            className="h-1.5 flex-1 rounded-full overflow-hidden bg-surface-50"
            role="progressbar"
