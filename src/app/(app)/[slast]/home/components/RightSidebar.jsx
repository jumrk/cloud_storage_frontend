...
export function RightSidebar({ overview, fileTypes, loading, children }) {
  ...
  if (loading) {
    return (
      <aside className="bg-white border-l border-gray-200 px-4 md:px-6 py-8 h-full sticky top-0 flex flex-col w-full md:max-w-[300px] md:min-w-[220px] md:block">
        <div className="mb-6 p-5 rounded-xl bg-white border border-gray-200 shadow-card">
        ...
        <div className="mb-2 p-5 rounded-xl bg-white border border-gray-200 shadow-card">
  ...
  return (
    <aside className="bg-white border-l border-gray-200 px-4 md:px-6 py-8 h-full sticky top-0 flex flex-col w-full md:max-w-[300px] md:min-w-[220px] md:block">
      <div className="mb-6 p-5 rounded-xl bg-white border border-gray-200 shadow-card">
        <div className="font-semibold text-gray-900 mb-3 text-lg tracking-wide">
          {t("home.sidebar.overview")}
        </div>
        <StoragePieChart used={overview.usedNum} total={overview.totalNum} />
      </div>
      <div className="mb-6 p-5 rounded-xl bg-white border border-gray-200 shadow-card">
        <div className="font-semibold text-gray-900 mb-3 text-base tracking-wide">
          {t("home.sidebar.total_storage")}
        </div>
        <div className="flex flex-col gap-2 text-sm">
          <div className="flex justify-between items-center py-1">
            <span className="text-gray-600">{t("home.sidebar.used")}</span>
      ...
      <div className="mb-2 p-5 rounded-xl bg-white border border-gray-200 shadow-card">
        ...
        <div className="grid grid-cols-2 gap-x-4 gap-y-2 text-sm">
          ...
          <div className="flex flex-col gap-0.5">
            <span className="text-gray-600">{t("home.sidebar.remain")}</span>
            <span className="font-bold text-base text-gray-900">
              {formatSize(Math.max(0, overview.totalNum - overview.usedNum))}
            </span>
          </div>
          <div className="col-span-2 flex flex-col gap-0.5 mt-2">
            <span className="text-gray-600">{t("home.sidebar.plan")}</span>
            <span className="font-bold text-base text-gray-900">
              {overview.plan}
            </span>
          </div>
