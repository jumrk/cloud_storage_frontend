...
export default function HardsubPage() {
  ...
  const handleFileSelect = (e) => {
    ...
    if (file && file.type.startsWith("video/")) {
      ...
      if (file.size > MAX_VIDEO_SIZE) {
        toast.error(
          `Kích thước video quá lớn. Tối đa ${MAX_VIDEO_SIZE / (1024 * 1024)}MB`
        );
        return;
      }
      setVideoFile(file);
  ...
  // Normalize region to fit within video bounds
  const normalizeRegion = (reg, videoWidth, videoHeight) => {
    if (!reg || !videoWidth || !videoHeight) return reg;

    let { x, y, width, height } = reg;
    ...
    // Ensure region doesn't exceed video bounds
    ...
    if (width < 20 || height < 20 || x < 0 || y < 0 || x >= videoWidth || y >= videoHeight) {
      return null;
    }
  ...
  const handleCanvasMouseMove = (e) => {
    ...
    if (dragAction === "draw") {
      ...
      const normalized = normalizeRegion(newRegion, videoWidth, videoHeight);
      if (normalized) {
      ...
      if (dragAction.includes("t")) {
        newRegion.y = Math.max(0, originalRegion.y + dy);
        newRegion.height = originalRegion.height - dy;
      }
      if (dragAction.includes("b")) {
        newRegion.height = Math.min(videoHeight - newRegion.y, originalRegion.height + dy);
      }
      if (dragAction.includes("l")) {
        newRegion.x = Math.max(0, originalRegion.x + dx);
        newRegion.width = originalRegion.width - dx;
      }
      if (dragAction.includes("r")) {
        newRegion.width = Math.min(videoWidth - newRegion.x, originalRegion.width + dx);
      }
      // Ensure minimum size
      const normalized = normalizeRegion(newRegion, videoWidth, videoHeight);
  ...
  const handleStartExtraction = async () => {
    ...
    // Validate region against video dimensions
    const video = videoRef.current;
    if (video && video.videoWidth && video.videoHeight) {
      ...
      if (!normalized) {
        toast.error(
          "Vùng phụ đề không hợp lệ. Vui lòng chọn lại vùng nằm trong video."
        );
        return;
      }
      ...
      // If video not loaded yet, send original region (backend will validate)
        ...
        <div className="space-y-4.5">
          ...
          <div className="bg-white border border-gray-200 rounded-lg shadow-sm">
            <div className="flex border-b border-gray-200">
              ...
              >
                <FiCrop className="text-base" /> Tách phụ đề
              </button>
              <button
              ...
              >
                <FiList className="text-base" /> Lịch sử
              </button>
            </div>
          </div>
          {/* Tab Content */}
          {activeTab === "extract" ? (
                  ...
                  <div className="relative bg-black rounded-lg overflow-hidden shadow-lg">
                    ...
                    <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
                      <button
                        ...
                        className="pointer-events-auto bg-white rounded-full p-4 hover:bg-white transition shadow-md"
                      >
                        {isPlaying ? (
            ...
            /* History Tab */ <div className="bg-white border border-gray-200 rounded-lg p-4 shadow-sm">
                      ...
                      >
                        ...
                        <div className="flex gap-2">
                          {job.outFiles?.some((f) => f.ext === "srt") && (
                            ...
                            >
                              <FiDownload className="text-xs" /> SRT
                            </button>
                          )}
                          {job.outFiles?.some((f) => f.ext === "vtt") && (
                            <button
                              ...
                              className="flex-1 flex items-center justify-center gap-1.5 px-2 py-1.5 bg-emerald-600 text-white rounded text-xs font-medium hover:bg-emerald-700 transition"
                            >
                              <FiDownload className="text-xs" /> VTT
                            </button>
                          )}
                        </div>
                      </div>
