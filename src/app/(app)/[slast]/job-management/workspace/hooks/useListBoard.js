...
export function useListBoard({ id, title, handleUpdate }) {
  ...
  const refresh = useCallback(async () => {
    try {
      ...
      toast.error(t("job_management.errors.load_list_error"));
    } finally {
      setLoading(false);
    }
  }, [getCardsByList, id, t]);
  ...
  const handleDeleteCard = async (cardId) => {
    try {
      ...
      if (!payload?.success) {
        toast.error(payload?.messenger || t("job_management.errors.delete_error"));
        return;
      }
  ...
  const submitAdd = async () => {
    ...
    try {
      ...
      setAddOpen(false);
      setLoading(false);
      toast.success(t("job_management.success.create_card_success"));
    } catch (error) {
  ...
  const patchCard = async (cardId, patch) => {
    ...
    try {
      ...
      toast.error(t("job_management.errors.update_failed"));
      throw e;
    }
  };
  ...
  const handleDragEnd = async (event) => {
    ...
    try {
      ...
      toast.success(t("job_management.success.sort_success"));
    } catch {
      setCards(prev);
      toast.error(t("job_management.errors.sort_failed"));
    }
  };
  ...
  const onDropNative = async (e) => {
    ...
    try {
      ...
      await refresh();
      toast.success(t("job_management.success.card_moved"));
    } catch {
      setCards((prev) => prev.filter((c) => String(c._id) !== String(cardId)));
      toast.error(t("job_management.errors.move_failed"));
    }
  };
      ...
      if (String(toListId) === String(id)) {
        setCards((prev) => {
          ...
          clone.splice(
            safePos,
            0,
            normalizeCard({ _id: cardId, title: t("job_management.errors.moving") })
          );
          return clone;
        });
      }
