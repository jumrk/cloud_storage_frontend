...
export default function CardTask({
  ...
  return (
    <div className="group relative w-full select-none rounded-xl border border-gray-200 bg-white p-3 hover:bg-white">
      <div className="flex items-start">
        <div className="min-w-0 pr-8">
          <h3 className="text-[14px] font-medium leading-snug text-gray-900 truncate">
            {title}
          </h3>
        ...
        <div className="absolute right-2 top-2 inline-flex">
          <button
            type="button"
            onClick={() => onEdit?.(id)}
            className="h-7 w-7 flex items-center justify-center rounded-full text-gray-600 hover:bg-white"
      ...
      {shouldShowDetails && hasLabels && (
        <div className="mt-2 flex flex-wrap gap-1.5">
          ...
          {labels.length > 6 && (
            <span className="inline-flex h-6 items-center rounded-md bg-white px-2 text-[11px] font-medium text-gray-900">
              +{labels.length - 6}
            </span>
          )}
        </div>
                ...
                <div className="flex -space-x-1.5">
                  {hasMembers ? (
                    <>
                      ...
                      {members.length > 5 && (
                        <span className="inline-flex h-6 w-6 items-center justify-center rounded-full bg-white text-gray-900 text-[11px] font-semibold ring-2 ring-white">
                          +{members.length - 5}
                        </span>
                      )}
                    </>
                  ) : (
                    <span className="inline-flex h-6 px-2 items-center justify-center rounded-full bg-white text-gray-600 text-[11px]">
                      Chưa gán
                    </span>
                  )}
                </div>
